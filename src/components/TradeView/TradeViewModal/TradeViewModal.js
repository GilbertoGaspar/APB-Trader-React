...
export default class TradeViewModal extends Component {
    ...
    handleSubmit = () => {
        ...
        if(this.state.validSearchTerm && hasEmptyData.length === 0) {
            let updatedSubmitData = this.state.submitData;
                ...
                updatedSubmitData['unixTimeExpires'] = updatedSubmitData['unixTimeCreated'] + this.timeToUnixTime(updatedSubmitData['time']);
                axios.post(FIREBASE_TRADE_URL, updatedSubmitData)
            .then(response => {
    ...
    handleAutoCompleteChange = (event) => {
        ...
        this.setState({ searchTerm: event.target.value }, () => {
            ...
            if(this.state.searchTerm.length >= 2) {
                axios.get(`https://apbdb.com/beacon/search?q=${this.state.searchTerm}&limit=15&cat=Weapon`)
                    .then(response => {
    ...
    render() {
        return (
            <div className="modal fade" id="tradeModalCenter" tabIndex="-1" role="dialog" aria-labelledby="tradeModalCenterTitle" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered" role="document">
                    <div className="modal-content">
                    <div className="modal-header">
                        <h5 className={`modal-title ${classes['text-black']}`} id="tradeModalLongTitle">Create Trade</h5>
                    ...
                    <div className={`modal-body ${classes['text-black']}`}>
                        <div className="input-group mb-3">
                            <div className="input-group-prepend">
                                <span className="input-group-text" id="basic-addon1">Weapon</span>
                            </div>
                            <input type="text" className="form-control" placeholder="Weapon Name" aria-label="Weapon Name" aria-describedby="basic-addon1" value={this.state.searchTerm}
                        ...
                        <div className="input-group mb-3">
                            ...
                            <input type="number" className="form-control" placeholder="Price" aria-label="Price" aria-describedby="basic-addon1" 
                                onChange={(event) => this.handleUpdateSubmitData('price', event.target.value)}
                        ...
                        <div className="input-group mb-3">
                            ...
                            <select className="custom-select" id="inputGroupSelect01" 
                                onChange={(event) => this.handleUpdateSubmitData('time', event.target.value)}>
                        ...
                        <div className="input-group">
                            <div className="input-group-prepend">
                                <span className="input-group-text" id="">Character and Server</span>
                            </div>
                            <input type="text" placeholder="Name" aria-label="Name" aria-describedby="basic-addon1" className="form-control" 
                                onChange={(event) => this.handleUpdateSubmitData('character', event.target.value)}/>
                            <select className="custom-select" id="inputGroupSelect01" 
                                onChange={(event) => this.handleUpdateSubmitData('server', event.target.value)}>
                                <option value="Jericho" defaultValue>Jericho</option>
