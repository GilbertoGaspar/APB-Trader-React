...
class TradeView extends Component {
    ...
    deleteOldTradesFromDataBase = (trades) => {
        trades.forEach(tradeKey => {
            axios.delete(`${FIREBASE_TRADE_URL_DELETE}${tradeKey}.json`)
    ...
    render() {
        ...
        if(this.state.trades !== null) {
            ...
            if(this.state.searchTerm === '') {
                ...
                trades = Object.entries(this.state.trades).map(trade => {
                    const { character, hudImage, price, server, unixTimeExpires, weapon } = trade[1];
                    let time = moment().to(unixTimeExpires, true);
                    return (
                        <TradeViewCell key={trade[0]} character={character} hudImage={hudImage} 
                                       price={price} server={server} time={time} 
                                       weapon={weapon} />
                    )
                });
            }
                ...
                trades = Object.entries(this.state.trades).filter(trade => {
                    return trade[1].weapon.toLowerCase().includes(this.state.searchTerm.toLowerCase());
                });
                //Create and display TradeViewCell per trade.
                trades = trades.map(trade => {
                    const { character, hudImage, price, server, time, unixTime, weapon } = trade[1];
                    return (
                        <TradeViewCell key={trade[0]} character={character} hudImage={hudImage} 
                                       price={price} server={server} time={time} unixTime={unixTime} 
                                       weapon={weapon} />
                    )
                });
            }
        ...
        if(this.state.currentUser) {
            tradeButton = <button type="button" className="btn btn-block btn-secondary" data-toggle="modal" data-target="#tradeModalCenter">Create Trade</button>;
        }
        return (
            <div className={classes['TradeView']}>
                <button className={`btn btn-secondary ${classes['TradeView__button']}`} id="btn-refresh" type="button" onClick={this.getLatestTrades}>Refresh</button>
                <h3>Current Trades</h3>
                {trades}
                <input type="text" className="form-control mb-2" placeholder="Weapon Search" aria-label="Weapon Search" aria-describedby="basic-addon1" 
                       onChange={(event) => this.handleSearchTermChanger(event)} />
                {tradeButton}
                <TradeViewModal user={this.state.currentUser.uid}/>
                <LoginModal register={this.handleRegister} login={this.handleLogin} logout={this.handleLogout} currentUser={this.state.currentUser}/>
